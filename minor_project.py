...
def driver(readl):
    ...
    print(
        "\nFirst line should contain the number of stations (n), the number of connected roads (any one direction) and the number of electric vehicles (k).")
    print(
    ...
    # n -> number of stations (nodes / vertices); m -> number of roads (edges); k -> number of vehicles
    n, m, k = map(int, readl[0].split(" "))
    period = int(readl[1])
    ...
    for i in range(k):
        a, b = map(int, readl[2+m+2*i].split(" "))
        c, d, e, f, g = map(float, readl[2+m+2*i+1].split(" "))
    ...
    for i in range(k):
        ...
        if solutionState:
            ...
            ss.extend(
            [" -> ".join(solutionState.vehicleStates[i].route),
            str(round(solutionState.vehicleStates[i].timestamp,2))]
            )
        else:
            print("Error computing optimal schedule and time. Please try varying the time period.\n")
            ss.append("Error computing optimal schedule and time. Please try varying the time period.\n")
        ...
        str(
                shortestPaths[vehicles[i].source][
                    vehicles[i].destination][
                    0]),
        str(round(vehicles[i].heuristicUnderestimate,2)) + "s",

        " -> ".join(shortestPaths[vehicles[i].source][
                    vehicles[i].destination][
                    1]),
        ss,
        ])
